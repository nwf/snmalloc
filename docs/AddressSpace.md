...
3. `BackendAllocator::alloc_chunk` at this point forks the allocation request in twain, allocating both the chunk's metadata structure (of size `PAGEMAP_METADATA_STRUCT_SIZE`) and the chunk itself (a multiple of `MIN_CHUNK_SIZE`).
...
4. The address is part of a live large allocation (spanning one or more 16KiB chunks):
   ...
   * has an associated `RemoteAllocator*` and metadata structure.
