...
namespace snmalloc
{
  ...
  static_assert(
    ...
    "ChunkRecord* must be pointer convertible to MetaCommon*");
  static_assert(
    sizeof(ChunkRecord) <= PAGEMAP_METADATA_STRUCT_SIZE,
    "Chunkrecord must fit in the pagemap metadata record");
